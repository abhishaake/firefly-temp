...
const WorkoutResourceCard: React.FC<{ 
  resource: { 
    id: string; 
  ...
  onAudioStop?: () => void;
  isPlaying?: boolean;
}> = ({ resource, onAudioPlay, onAudioStop, isPlaying }) => {
  const [isLoading, setIsLoading] = useState(false);
  const mediaService = new MediaService();

  const handleClick = async () => {
    if (!resource.audioFileId) return;
    ...
    try {
      setIsLoading(true);
      const response = await mediaService.getFileUrl(resource.audioFileId);
      if (response?.data) {
        onAudioPlay?.(response.data);
      }
    } catch (error) {
  ...
  return (
    <div 
      className={`workout-resource-card ${isPlaying ? 'playing' : ''} ${isLoading ? 'loading' : ''}`}
      onClick={resource.audioFileId ? handleClick : undefined}
      style={{ cursor: resource.audioFileId ? 'pointer' : 'default' }}
    >
      <div className="workout-resource-icon-wrapper">
        {isLoading ? (
...
}> = ({ data, onAudioPlay, onAudioStop, currentPlayingId }) => {
  const navigate = useNavigate();
  const isPlaying = currentPlayingId === `workout-${data.workoutId}`;
      ...
      <div className="figma-workout-card-level-row" style={{ justifyContent: 'space-evenly' }}>
        {(
          <WorkoutResourceCard 
            resource={{
              id: "1", 
              icon: <div><svg width="15" height="17" viewBox="0 0 15 17" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              filename: data.audioFileId ? "Music" : "No Music attached",
        ...
        {/* <WorkoutResourceCard resource={{
          ...
          </svg></div>, filename: "visual.xml"
        }} /> */}
      </div>

      <div className="figma-workout-card-actions-row">
        <span className="figma-workout-card-meta">Created by: {data.createdBy}</span>
...
export const WorkoutPage: React.FC = () => {
  ...
  const handleWorkoutAudioPlay = (workoutId: number, audioUrl: string) => {
    setCurrentPlayingId(`workout-${workoutId}`);
    handleAudioPlay(audioUrl);
  };

  const handleWorkoutAudioStop = () => {
    handleAudioStop();
  };
      ...
      <div className="figma-workout-list">
        ...
        {Array.isArray(workouts) && workouts.map((workout: Workout) => (
          <WorkoutCard 
            ...
            data={workout}
            onAudioPlay={(audioUrl) => handleWorkoutAudioPlay(workout.workoutId, audioUrl)}
            onAudioStop={handleWorkoutAudioStop}
